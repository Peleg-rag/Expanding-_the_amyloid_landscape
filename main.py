...
import jpred
import tango
from config import PROJECT_PATH, JOB_RUN, LANDAU_LAB_PEPTIDES_FILEPATH, RUN_ONLY_DATABASE, KEY

SEPARATOR = "\n********************************************************************************************************"
EXTERNAL_RESULT_DIRNAME = 'results'
LOCAL_RESULT_DIRNAME = 'Output'
...
def create_database(database_filepath: str, statistical_result_dict: dict, database_name: str) -> pd.DataFrame:
    ...
    if 'Length' not in original_database.columns:
        original_database["Length"] = original_database["Sequence"].str.len()

    filtered_database = auxiliary.filter_uncertain_and_long_sequences(original_database)

    filtered_database = auxiliary.filter_entries_rejected_by_Jpred(filtered_database)

    statistical_result_dict[database_name]["After filter"] = filtered_database.shape[0]

    auxiliary.correct_database_sequences(filtered_database)

    # write_to_excel(filtered_database, JOB_RUN + "/Filtered_" + database_name + ".xlsx")
    # assert len(filtered_database['Sequence'].unique()) == filtered_database.shape[0], \
    #     "!!! There are duplicated sequences in the database !!!"
...
def run_and_analyse_jpred(database: pd.DataFrame, database_name: str, job_name: str):
    """
    ...
    :return: Adds columns to the given database
    """
    num_new_peptides_to_run_jpred = jpred.creat_jpred_input(database, database_name, job_name)
    if num_new_peptides_to_run_jpred == 0:
        jpred.process_jpred_output(database, database_name, JOB_RUN)
...
def perform_fibril_formation_prediction(database: pd.DataFrame, database_name: str, statistical_result_dict: dict):
    ...
    for _, row in database.iterrows():
        ...
        if row["Helix score (Jpred)"] != -1 and row["Helix (Jpred) uH"] >= jpred_helix_avg_uH:
...
def add_final_prediction_to_statistical_result_dict(statistical_result_dict: dict, database: pd.DataFrame,
    ...
    ff_helix_count = 0
    ff_ssw_count = 0
    ff_helix_or_ssw_count = 0
    ff_helix_and_ssw_count = 0
    count_antimicrobial_ff_results = 0
    count_antimicrobial_ff_AND_results = 0
    antimicrobial_count = 0

    for _, row in database.iterrows():
        if row['SSW prediction'] == 1:
            ssw_count += 1
        if row['Helix score (Jpred)'] != -1:
            helix_count += 1
        if row["FF-Secondary structure switch"] == 1:
            ff_ssw_count += 1
        if row["FF-Helix (Jpred)"] == 1:
            ff_helix_count += 1
        if (row["FF-Secondary structure switch"] == 1) or (row["FF-Helix (Jpred)"] == 1):
            ff_helix_or_ssw_count += 1
            if row["Entry"] in antimicrobial_entries:
                count_antimicrobial_ff_results += 1
        if (row["FF-Secondary structure switch"] == 1) and (row["FF-Helix (Jpred)"] == 1):
            ff_helix_and_ssw_count += 1
            if row["Entry"] in antimicrobial_entries:
                count_antimicrobial_ff_AND_results += 1
        if row[KEY] in antimicrobial_entries:
            antimicrobial_count += 1

    statistical_result_dict[database_name]["# Helix prediction"] = helix_count
    statistical_result_dict[database_name]["# FF Helix prediction"] = ff_helix_count

    statistical_result_dict[database_name]["# SSW prediction"] = ssw_count
    statistical_result_dict[database_name]["# FF SSW prediction"] = ff_ssw_count

    statistical_result_dict[database_name]["3 FF helix OR SSW prediction"] = ff_helix_or_ssw_count
    statistical_result_dict[database_name]["3 FF helix AND SSW prediction"] = ff_helix_and_ssw_count

    statistical_result_dict[database_name]["Antimicrobial annotation"] = antimicrobial_count
    statistical_result_dict[database_name]["Antimicrobial FF Helix OR SWW"] = count_antimicrobial_ff_results
    statistical_result_dict[database_name]["Antimicrobial FF Helix AND SWW"] = count_antimicrobial_ff_AND_results
...
if __name__ == "__main__":
    local_result_dirpath = os.path.join(PROJECT_PATH, LOCAL_RESULT_DIRNAME)
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print(local_result_dirpath)
    print("local_result_dirpath")
    os.makedirs(local_result_dirpath, exist_ok=True)

    external_result_dirpath = os.path.join(PROJECT_PATH, EXTERNAL_RESULT_DIRNAME)
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print(external_result_dirpath)
    print("external_result_dirpath")
    os.makedirs(external_result_dirpath, exist_ok=True)
    ...
    if len(RUN_ONLY_DATABASE) > 0:
        ...
        if JOB_RUN == "Uniprot_Keywords":
            database_name_index_after_split = 3
        else:
            database_name_index_after_split = 4
    print(*input_file_list, sep='\n')

    statistical_result_dict = {}

    antimicrobial_entries_list = auxiliary.get_all_antibacterial_entries()
    ''' if user dosent want to find antimicrobial entries inside each database remove note from next line '''
    ...
    for file in input_file_list:
        print(SEPARATOR)
        ''' if the user gives a job name, than the database_name should be defined to be that job name '''
        database_name = auxiliary.get_database_name_from_file(file, JOB_RUN)
        ...
        # print(cur_database.loc[8, 'Entry'])
        print()

        print("     Creating_jpred_input........................................................")
        run_and_analyse_jpred(cur_database, database_name, JOB_RUN)
        ...
        cur_database_final_result_filepath = os.path.join(local_result_dirpath, f"Final_{database_name}.xlsx")
        final_results_filepath = os.path.join(external_result_dirpath, f"Final_{database_name}.xlsx")
        write_to_excel(cur_database, cur_database_final_result_filepath)
        write_to_excel(cur_database, final_results_filepath)

    write_to_excel(pd.DataFrame(statistical_result_dict),
                   os.path.join(external_result_dirpath, "Final_statistical_result.xlsx"),
                   True)
    write_to_excel(pd.DataFrame(statistical_result_dict),
                   os.path.join(local_result_dirpath, "Final_statistical_result.xlsx"),
                   True)
