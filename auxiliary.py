...
# Predefined averaged score for predicting segments with defined secondary structure by Tango.
# This parameter is used only internally and shouldn't be modified.
MIN_TANGO_SCORE = 0
...
def get_input_files(run_job: str) -> list:
    ...
    """
    input_files = []
    for root, dirs, files in os.walk(config.PROJECT_PATH + '/' + run_job + '/Database'):
        for file in files:
            input_files.append(run_job + '/Database/' + file)
    return input_files


def filter_uncertain_and_long_sequences(database: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    mask = database["Length"] <= config.MINIMAL_PEPTIDE_LENGTH

    if "Sequence caution" in database.columns:
        mask &= database["Sequence caution"].isnull()

    if "Sequence conflict" in database.columns:
        mask &= database["Sequence conflict"].isnull()

    if "Sequence uncertainty" in database.columns:
        mask &= database["Sequence uncertainty"].isnull()
...
def __check_subsegment(prediction: list, start: int, end: int) -> tuple:
    all_possible_length = [i for i in range(config.MIN_SEGMENT_LENGTH, (end - start + 1 + 1))]
    max_start = -1
    max_end = -1
    max_score = -1
...
def get_secondary_structure_segments(prediction: list, prediction_method: str) -> list:
    ...
    if prediction_method == "Tango":
        ...
        min_score = config.MIN_JPRED_SCORE
    ...
    while i < len(prediction):
        if prediction[i] > 0:
            ...
            good_segment = segment_length >= config.MIN_SEGMENT_LENGTH and (mean(prediction[start:end]) >= min_score or
                                                                            median(prediction[start:end]) >= min_score)
            if good_segment:
                segments.append(tuple((start, end)))
            elif segment_length >= config.MIN_SEGMENT_LENGTH:
...
def get_all_antibacterial_entries() -> list:
    amps_database_filepath = os.path.join(config.PROJECT_PATH,
                                          "Uniprot_keywords/Database/uniprot-keyword__Antimicrobial+[KW-0929]_.xlsx")
    with warnings.catch_warnings(record=True):
        warnings.simplefilter("always")
        original_database = pd.read_excel(amps_database_filepath, engine="openpyxl")
...
def add_database_info_to_statistical_results(statistical_result_dict: dict):
    # C:/Users/peleg/PycharmProjects/Expanding_Amyloid_Landscape/
    uniprot_kb_filepath = os.path.join(config.PROJECT_PATH, 'Uniprot_KB/Uniprot_KB_databases_info.xlsx')
    uniprot_kb_databases_info = pd.read_excel(uniprot_kb_filepath, engine='openpyxl')
...
def get_database_name_from_file(file_name: str, job_name: str) -> str:
    """
    ...
    """
    # TODO: for different filenames it will need to change
    database_file_split = file_name.split('_')
    if job_name == 'Uniprot_keywords':
        # print(database_file_split[3])
        # print(database_file_split[3].split('['))
        return database_file_split[3].split('[')[0]
    return database_file_split[4] + '_' + database_file_split[5]


def filter_entries_rejected_by_Jpred(database: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    """
    filtered_database = database.drop(database[database[config.KEY] == 'A0A3D2B8F4'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A6B2FE55'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A1X4JA79'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A1X4JF71'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A0E1PXY5'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A4NWW5'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A5UHP6'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A377Z9E9'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A8H5QVH7'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A1V3AC84'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A2T6QXC9'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'A0A438WIG1'].index)
    filtered_database = filtered_database.drop(filtered_database[filtered_database[config.KEY] == 'Q9RE35'].index)
    return filtered_database


def correct_database_sequences(database: pd.DataFrame):
    """
    This function corrects sequences with general amino acids by these set of rules:
    "X" -> "A"
    "Z" -> "E"
    "B" -> D
    "U" -> C
    ...
    :param database:
    :return: Changes the sequence inside the dataframe, returns nothing
    """

    for idx, row in database.iterrows():
        database.at[idx, 'Sequence'] = get_corrected_sequence(row['Sequence'])
