...
TANGO_DIRECTORY = os.path.join(PROJECT_PATH, "Tango")
TANGO_BAT_FILEPATH = os.path.join(TANGO_DIRECTORY, "Tango_run.bat")
KEY = "Entry"
# TANGO_RESULT_DIRECTORY = "C:/Users/peleg/PycharmProjects/Expanding_Amyloid_Landscape/Tango"
...
def __get_peptide_tango_result(filepath: str) -> dict:
    ...
    peptide_tango_result_dict = {"Name": os.path.splitext(os.path.basename(filepath))[0],
...
def __get_database_tango_results(database_entries: list) -> dict:
    ...
    """
    tango_result_directory = os.fsencode(TANGO_DIRECTORY)
    tango_results_dict = {}
    for file in os.listdir(tango_result_directory):
        filename = os.fsdecode(file)
        if "Tango" not in filename:
            filepath = os.path.join(TANGO_DIRECTORY, filename)
...
def process_tango_output(database: pd.DataFrame):
    ...
    for _, row in database.iterrows():
        ...
        if row[KEY] == 'A0A1D5AB43':
            print()
            print('?????????????????????????????????????????????????????????????????????????????????????????????????')
            print(peptide_tango_results)
            print('?????????????????????????????????????????????????????????????????????????????????????????????????')
            print()
...
def run_tango():
    ...
    p = subprocess.Popen("Tango_run.bat", cwd=TANGO_DIRECTORY, shell=True)
...
def get_all_existed_tango_results_entries() -> set:
    ...
    for file in os.listdir(TANGO_DIRECTORY):
        filename = os.fsdecode(file)
        if "Tango" not in filename:
            uniprot_id = filename.split('.')[0]
